...
const createAnthropicHandler = ({
  client, model, systemPrompt, tools = [], maxIterations = 10, maxTokens = 4096
}) => {
  ...
  return async function* (input) {
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        switch (event.type) {
          ...
          case "content_block_start": {
            ...
            if (cb.type === "thinking") {
              ...
              openToolUse.set(event.index, { id: cb.id, name: cb.name, inputJson: "" });
            }
            break;
          }
          ...
          case "content_block_delta": {
            ...
            if (d.type === "text_delta") {
              ...
              if (tid) yield { type: "thinking_delta", thinkingId: tid, delta: d.thinking };
          ...
          case "content_block_stop": {
            ...
            if (tu) {
              let parsedInput = {};
              if (tu.inputJson) {
                try { parsedInput = JSON.parse(tu.inputJson); } catch { /* leave {} */ }
              }
              yield {
      ...
      for (const tu of toolUses) {
        ...
        yield error
          ? { type: "tool_call_end", toolCallId: tu.id, error, durationMs: Date.now() - started }
          : { type: "tool_call_end", toolCallId: tu.id, output, durationMs: Date.now() - started };
...
const turns = [
  {
    ...
    events: [
      ...
      { type: "content_block_delta", index: 2, delta: { type: "text_delta", text: "Looking now..." } },
  ...
  },
  {
    events: [
      ...
      { type: "content_block_start", index: 0, content_block: { type: "text", text: "" } },
      { type: "content_block_delta", index: 0, delta: { type: "text_delta", text: "Result: 42." } },
      { type: "content_block_stop", index: 0 },
    ...
    execute: async ({ q }) => {
      ...
      return {
        ...
        sources: [{
          ...
          title: "Knowledge doc", url: "https://example.com/doc",
...
console.log("Stream:");
for (const e of events) console.log(" ", e.type, JSON.stringify(e).slice(0, 100));
...
const tokensOk = me?.type === "message_end" && me.tokenUsage?.totalTokens === expectedTotal;
...
const streamingOk =
  beforeStart?.type === "content_delta" && beforeStart.delta === "Let me check. " &&
  afterStart?.type === "content_delta" && afterStart.delta === "Looking now...";
...
console.log("tool round-trip:   ", toolOk ? "PASS" : `FAIL (calls=${lookupCalls}, input=${JSON.stringify(toolCallStart?.input)}, output=${JSON.stringify(toolCallEnd?.output)})`);
console.log("sources batched:   ", sourcesOk ? "PASS" : `FAIL (${JSON.stringify(sourcesEvent)})`);
console.log("streamed mid-turn: ", streamingOk ? "PASS" : `FAIL (before=${JSON.stringify(beforeStart)}, after=${JSON.stringify(afterStart)})`);
