...
Run your agent inside a Temporal workflow. [`@sharely/adapter-temporal`](../../packages/adapter-temporal/) starts the workflow per chat turn and polls its event-buffer query until `message_end`. The workflow side emits `AgentEvent`s via `createAgentEventSink` + `emitAgentEvent`.
...
export const sharelyAgentWorkflow = async (
  ...
  emitAgentEvent(sink, { type: 'message_start', role: 'assistant', model: 'my-model' });
  ...
  emitAgentEvent(sink, { type: 'content_delta', delta: `Echo: ${input.message}` });
...
Sharely's server persists message history in `agentMessage`; the workflow doesn't need to. The workflow receives the prior turns via `input.history` and emits its events; `@sharely/server` handles the rest.
