...
//
// Identical in shape to `live-demo-temporal`'s handler — the client side doesn't
// care whether the workflow hand-rolls a fetch loop or uses the AI SDK plugin.
// `fromTemporal` turns a Temporal workflow into a Sharely `Handler`: on each chat
// turn it `client.start()`s a workflow execution and polls its
// `AGENT_EVENTS_QUERY` event-buffer query until the workflow emits `message_end`,
// yielding AgentEvents as they arrive. Client disconnect (`input.signal`) cancels
// the workflow.
//
...
export const createTemporalHandler = (opts: TemporalHandlerOptions): Handler =>
  fromTemporal({
    client: opts.client,
    workflowType: opts.workflowType ?? 'sharelyAiSdkAgentWorkflow',
    taskQueue: opts.taskQueue ?? 'sharely-agents-ai-sdk',
