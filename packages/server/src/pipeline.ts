...
interface Accumulator {
  ...
  sources: Source[];
  tokenUsage?: TokenUsage; model?: string; finishReason: string;
}

const reduce = (a: Accumulator, e: AgentEvent): void => {
  switch (e.type) {
    case "message_start": a.model = e.model; return;
    case "content_delta": a.content += e.delta; return;
    case "thinking_start":
      a.thinking.set(e.thinkingId, { id: e.thinkingId, title: e.title, content: "", status: "in_progress" });
      return;
    case "thinking_delta": {
      const s = a.thinking.get(e.thinkingId); if (s) s.content += e.delta; return;
    }
    case "thinking_end": {
      const s = a.thinking.get(e.thinkingId);
      if (s) { s.status = e.status; s.durationMs = e.durationMs; } return;
    }
    case "tool_call_start":
      a.toolCalls.set(e.toolCallId, { id: e.toolCallId, name: e.name, input: e.input, status: "in_progress" });
      return;
    case "tool_call_end": {
      const c = a.toolCalls.get(e.toolCallId);
      if (c) { c.output = e.output; c.error = e.error; c.status = e.error ? "error" : "completed"; c.durationMs = e.durationMs; }
      return;
    }
    case "sources": a.sources = e.sources; return;
    case "message_end": a.finishReason = e.finishReason; a.tokenUsage = e.tokenUsage; return;
    case "content_end":
    case "error": return;
  }
};
...
export const runHandler = async ({ handler, context, message, res, api }: RunOptions): Promise<void> => {
  ...
  const acc: Accumulator = { content: "", thinking: new Map(), toolCalls: new Map(), sources: [], finishReason: "stop" };
  let errored = false;
  ...
  try {
    ...
    if (!res.writableEnded) {
      sendSSEEvent(res, "error", { ...envelope, error: err instanceof Error ? err.message : "Handler error" });
    }
  }
  ...
  if (!res.writableEnded) { sendDone(res, envelope); endSSEStream(res); }
};
