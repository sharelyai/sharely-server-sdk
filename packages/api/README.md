...
Inside `@sharelyai/server`, the chat route builds one of these per request and exposes it on `AgentContext.api` so your `Handler` can call platform endpoints itself.
...
```ts
import { createSharelyAPIClient, type Transport } from '@sharelyai/api';
