...
import type { AgentContext, ToolContext } from "@sharely/protocol";
...
/**
 * `@sharely/adapter-vercel-ai/tools` — the first-party Sharely tools wrapped as
 ...
 * backed by the platform (`@sharely/api.rag()`) out of the box; the others have
...
const sharelyTool =
  (name: string) =>
  (context: AgentContext, executor?: ToolExecutor) => {
