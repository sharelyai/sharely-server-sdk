...
import { scenarios, runHandlerConformance } from '@sharelyai/conformance';
...
console.log(
  `${abortOk ? 'PASS' : 'FAIL'}  abort-signal halts stream (${abortEvents.length} events)`,
);
...
const toolContext = {
  ...
  api: {
    ...
    rag: async () => {
      ragCalls++;
      return [{ id: 'k1', score: 0.9, metadata: { title: 'Doc' } }];
    },
  },
  trace: {
    traceId: 'tr',
...
console.log(
  `${semOk ? 'PASS' : 'FAIL'}  /tools semanticSearch → Vercel tool backed by api.rag()`,
);
...
console.log(
  `${skOk ? 'PASS' : 'FAIL'}  /tools searchKnowledge reports missing executor`,
);
if (!skOk) allOk = false;
