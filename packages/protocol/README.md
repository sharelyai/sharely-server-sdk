...
- **`Handler` / `AgentInput` / `AgentContext`** — the single function shape every agent must implement. Pattern C (raw `Handler`), `@sharely/adapter-vercel-ai`, and `@sharely/adapter-temporal` all produce this.
- **`Source` / `ThinkingStep` / `ToolCallRecord` / `TokenUsage`** — the domain payload types the server persists into the `agentMessage` row.
- **`Tool` / `ToolDefinition` / `ToolContext` / `ToolResult`** — the LLM-facing tool contract. `@sharely/tools` extends this with the 7 first-party definitions.
- **`SharelyAPIClient`** (stub) — narrow shape `AgentContext.api` satisfies. The real client lives in `@sharely/api`.
- **`TraceSpan`** (stub) — minimal trace shape; concrete impls live in `@sharely/server` and adapters.
